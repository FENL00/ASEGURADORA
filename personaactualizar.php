...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($anos < 18 || $anos > 64) {
        ...
        $operacion->actualizarpersona($id_usua, $fechanacimiento, $ciudad, $direccion, $empresa, $cargo, $celularempresa, $ingreso, $egreso, $tiposeguro, $estadoseguro, $id);
    ...
    <div class="container">
        <div class="card ">
            <div class="card-header text-white bg-primary">
                ...
                <?php if ($id_rol == 1) {
                ?>
                <a href="personalistar.php" class="btn btn-primary bg-info">lLista de Seguro de Persona</a>
            </div>
            <?php
                } else {
                     ?>
        ...
        <div class="card-body">
            ...
            <form ROLE="FORM" METHOD="POST" ACTION="">
                ...
                <?php
                        ...
                        ?>
                <?php
                    $pers = $operacion->buscarpersona($idrul);
                    ?>
                <input type="hidden" class="form-control" id="id" name="id"
                    value="<?php echo !empty($pers['p_id']) ? $pers['p_id'] : ''; ?>">
                <input type="hidden" class="form-control" id="id_usua" name="id_usua"
                    value="<?php echo !empty($pers['p_usuario']) ? $pers['p_usuario'] : ''; ?>">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="ingreso" name="ingreso"
                        placeholder="Ingreso Mensuales"
                        value="<?php echo !empty($pers['p_ingreso']) ? $pers['p_ingreso'] : ''; ?>" required>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="estadoseguro" name="estadoseguro">
                        <?php
                            ...
                            if ($pers['p_estadoseguro'] == '') {
                                foreach ($estseg as $i => $valor) {
                            ?>
                        <option value="<?php echo $estseg[$i] ?>">
                            <?php echo $estseg[$i] ?>
                        </option>
                        <?php
                                }
                                ...
                                foreach ($estseg as $i => $valor) {
                                    if ($estseg[$i] == $pers['p_estadoseguro']) {
                                       ?>
                        <option value="<?php echo $estseg[$i] ?>" selected>
                            <?php echo $estseg[$i] ?>
                        </option>
                        <?php
                                    } else {
